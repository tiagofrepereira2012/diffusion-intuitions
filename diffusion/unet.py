"""
For the U-Net I basically took it from their code.

This U-Net is a bit different from the original U-Net that I'm familiar with.
It has the residual blocks and the Self-Attention blocks, but the mechanism is the same.

There is also this timestep embedding that I need to understand better.

I might revisit this later to see if I can make it simpler and more readable (at least for me).
"""
...
class UNet(nn.Module):
    ...
    def forward(self, x, t):
        ...
        # Down
        x2 = self.down[0](x1, temb)  # ResNetBlock
        x3 = self.down[1](x2, temb)  # ResNetBlock
        x4 = self.down[2](x3)  # Downsample
        x5 = self.down[3](x4, temb)  # ResNetBlock
        x6 = self.down[4](x5)  # Attention
        x7 = self.down[5](x6, temb)  # ResNetBlock
        x8 = self.down[6](x7)  # Attention
        x9 = self.down[7](x8)  # Downsample
        x10 = self.down[8](x9, temb)  # ResNetBlock
        x11 = self.down[9](x10, temb)  # ResNetBlock
        x12 = self.down[10](x11)  # Downsample
        x13 = self.down[11](x12, temb)  # ResNetBlock
        x14 = self.down[12](x13, temb)  # ResNetBlock

        # Middle
        x = self.middle[0](x14, temb)  # ResNetBlock
        x = self.middle[1](x)  # Attention
        x = self.middle[2](x, temb)  # ResNetBlock

        # Up
        x = self.up[0](torch.cat((x, x14), dim=1), temb)  # ResNetBlock
        x = self.up[1](torch.cat((x, x13), dim=1), temb)  # ResNetBlock
        x = self.up[2](torch.cat((x, x12), dim=1), temb)  # ResNetBlock
        x = self.up[3](x)  # Upsample
        x = self.up[4](torch.cat((x, x11), dim=1), temb)  # ResNetBlock
        x = self.up[5](torch.cat((x, x10), dim=1), temb)  # ResNetBlock
        x = self.up[6](torch.cat((x, x9), dim=1), temb)  # ResNetBlock
        x = self.up[7](x)  # Upsample
        x = self.up[8](torch.cat((x, x8), dim=1), temb)  # ResNetBlock
        x = self.up[9](x)  # Attention
        x = self.up[10](torch.cat((x, x6), dim=1), temb)  # ResNetBlock
        x = self.up[11](x)  # Attention
        x = self.up[12](torch.cat((x, x4), dim=1), temb)  # ResNetBlock
        x = self.up[13](x)  # Attention
        x = self.up[14](x)  # Upsample
        x = self.up[15](torch.cat((x, x3), dim=1), temb)  # ResNetBlock
        x = self.up[16](torch.cat((x, x2), dim=1), temb)  # ResNetBlock
        x = self.up[17](torch.cat((x, x1), dim=1), temb)  # ResNetBlock
